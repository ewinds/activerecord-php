...
class Generator
{
  public static function generate($stub_models_dir)
  {
    ...
    $generated_models_dir = $stub_models_dir;
    ...
    foreach ($tables as $table_row) {
      ...
      if ($db_config['AR_PREFIX'] && $db_config['AR_PREFIX'] != '')
        $table_vanity_name = preg_replace('/^' .$db_config['AR_PREFIX']. '/', '', $table_name);

      if (is_array($db_config['AR_TABLES'])) {
        if (!in_array($table_vanity_name, $db_config['AR_TABLES']))
          continue;
      }
